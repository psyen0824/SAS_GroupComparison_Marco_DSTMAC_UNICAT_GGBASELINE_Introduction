...
7.20181010 Add P value control
6.20180929 Fixed exmissing in chisq test
	  ...
	  %else  %do;
		    ...
		    %let rc = %sysfunc(close(&dsid.));
		     %if &checkvar. = 0 %then   %do;
               %put  ERROR: Variable &grp is not exists!; 
               %abort; 
             %end;
	 %end;
...
%macro descB2(data=, var=,vtype=, grp=, test=,n=);
  %if %upcase(&test) in CHISQ  CMH  TREND FISHER  %then %do;
   %desc_ctg_b2(data=&data, var=&var,vtype=&vtype, grp=&grp,n=&n, exmissing=&exmissing);
  %end;
...
%macro desc_ctg_b2(data=, var=, vtype=, grp=,n=, exmissing=&exmissing);
	proc freq data=&data noprint;
	    table &var*&grp/missing nopercent outpct out=desc&n._n;
		
	    %if %upcase(&exmissing) eq Y %then %do;
...
%macro desc_ctn_b2(data=, var=, grp=, n=);
	...
	data _null_;
	  set hl&n;
	  *call symputx("hl&n", cat(_HL_," (",L_HL," to ", U_HL,")"));
	...
	data desc&n(keep= &grp  nnmiss meanstd  minmax  medianIQR);
		...
		minmax = cats(put(&var._min,12.1),"-", put(&var._max,12.1));
		meanstd = cats(put(&var._mean,12.1), "±", put(&var._stddev,12.1));
		* meanstd = cats(put(&var._mean,12.1), byte(177), put(&var._stddev,12.1));
		medianIQR = compress(put(&var._median,6.1))|| " ("||compress(put(&var._q1,6.1))||"-"||compress(put(&var._q3,6.1))||")";
	...
	%if  &ngrplabel EQ  2 and &stdiff EQ Y  %then %do;
	     data desc&n;
           set desc&n;
           if _name_="meanstd"	 then stdiff= abs(100* ((input(scan(col1,1,"±"),12.1)-input(scan(col2,1,"±"),12.1))/sqrt(( input(scan(col1,2,"±"),12.1)**2 + input(scan(col2,2,"±"),12.1)**2 )/2)));
	       * if _name_="meanstd" then stdiff= 100* ((input(scan(col1,1, byte(177)),12.1)-input(scan(col2,1, byte(177)),12.1))/sqrt(( input(scan(col1,2, byte(177)),12.1)**2 + input(scan(col2,2,byte(177)),12.1)**2 )/2));
...
%macro freq(data=, var=, vtype=, grp=, test=, n=);
	proc freq data=&data noprint;
		...
		%if %upcase(&exmissing) eq Y %then %do;
			table &var*&grp/norow nocol nopercent &test;
		%end;
		%else %do;
			table &var*&grp/missing norow nocol nopercent &test;
		%end;
...
%macro merge_cate_b2(var=, varlabel=, n=);
   ...
   data merge&n;
	 ...
	 *set dslabel&n desc&n;
	 set dslabel&n desc&n(where=(not missing(col0)));   
  ...
  data dslabel&n;
	...
	*set dslabel&n desc&n;
	...
	select;
		...
		when(_NAME_ = 'minmax')     label = "^{nbspace 6}Min-Max";
		when(_NAME_ = 'meanstd')    label = "^{nbspace 6}Mean±SD";
...
proc report data=&data nowindows spacing=1 headline headskip split = "|" ; 
 ...
 %if %upcase(&totcol) eq Y %then %do; 
	...
	%do c=0 %to %sysevalf(&ngrp-1);
	  * define col&c /display right  %sysfunc(compress("&&grplabel&c.|(N=&&n&c)")) ;
    ...
    define  stdiff /display right  "Absolute|Standardized Difference"  f=6.1; 
	%end;
 %end;
  ...
  %if %upcase(&totcol) eq N %then %do; 
    ...
    %if &ngrplabel EQ  2 and  %upcase(&stdiff) EQ Y %then  %do;
    define  stdiff /display right  "Absolute standardized |difference/Hodges-Lehmann estimator"  f=6.1; 
	%end;
 %end;
	...
	*footnote2 j=left height=10pt "^{nbspace &fnspace}Created by %nrstr(%ggBaseline1) in SAS &SYSVER in &SYSSCPL on &sysdate9";
