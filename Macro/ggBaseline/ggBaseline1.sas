...
%macro descB1(data=, var=, vtype=, vartype=,varlabel=,n=);
  %if %upcase(&vartype) EQ CTG  %then %do;
   %desc_ctg_b1(data=&data, var=&var,vtype=&vtype, varlabel=&varlabel,n=&n, exmissing=&exmissing)
  %end;
...
%macro desc_ctg_b1(data=, var=, vtype=, varlabel=, n=, exmissing=&exmissing);
...
%macro desc_ctn_b1(data=, var=, varlabel=, n=);
	...
	data desc&n(keep= nnmiss meanstd  minmax  medianIQR);
		...
		* nnmiss = compress(put(&var._n,6.))||" ("||compress(put(&var._nmiss,6.))||")";
		
		if &var._nmiss ^=0 then do;
		nnmiss = compress(put(&var._nmiss,6.))||" ("||compress(put(&var._nmiss/nobs*100,4.1))||")";
	    end; 
		
		minmax = cats(put(&var._min,12.1),"¨C", put(&var._max,12.1));
		* meanstd = cats(put(&var._mean,12.1), byte(177), put(&var._stddev,12.1));
		meanstd = cats(put(&var._mean,12.1), "¡À", put(&var._stddev,12.1));
   ...
   data merge&n;
	...
	select;
		...
		when(_NAME_ = 'minmax')     label = "^{nbspace 6}Min-Max";
		 when(_NAME_ = 'meanstd')    label = "^{nbspace 6}Mean¡ÀSD";
		 * when(_NAME_ = 'meanstd')    label = "^{nbspace 6}Mean %sysfunc(byte(177)) SD";
		 when(_NAME_ = 'medianIQR')  label = "^{nbspace 6}Median (Q1-Q3)";
		otherwise;
	end;
	order=&n;
  run;
